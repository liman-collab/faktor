...
class WeightCalculationSettings implements Hookable {
	...
	public function hooks() {
		add_filter( 'flexible_shipping_method_settings', array( $this, 'append_weight_calculation_settings' ) );
		add_action( 'flexible-shipping/method-rules-settings/table/after', array( $this, 'add_validator' ) );
	}
	...
	public function append_weight_calculation_settings( $settings_fields ) {
		...
		foreach ( $settings_fields as $key => $settings_field ) {
			...
			if ( 'method_debug_mode' === $key ) {
				$new_settings_fields['weight_ratio'] = array(
	...
	private function get_docs_url() {
		if ( get_user_locale() === 'pl_PL' ) {
			return 'https://www.wpdesk.pl/docs/flexible-shipping-pro-woocommerce-docs/?utm_source=volume-weight&utm_medium=link&utm_campaign=fs-method#wspolczynnik-przeliczeniowy';
		}

		return 'https://docs.flexibleshipping.com/article/505-flexible-shipping-shipping-method-advanced-options?utm_source=volume-weight&utm_medium=link&utm_campaign=fs-method';
	}
}
