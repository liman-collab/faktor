...
class MaxDimension extends AbstractCondition {
	...
	public function __construct( $priority = 10 ) {
		...
		$this->description  = __( 'Shipping cost based on the product\'s maximum dimension allowed', 'flexible-shipping-pro' );
	...
	private function format_input_data_for_logger( array $contents ) {
		...
		foreach ( $contents as $item ) {
			$dimensions[] = $this->get_product_max_dimension( $item['data'] );
		}
