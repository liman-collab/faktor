...
class DimensionalWeight extends AbstractCondition {
	...
	public function __construct( $priority = 10 ) {
		...
		$this->group        = __( 'Weight & Dimensions', 'flexible-shipping-pro' );
	...
	public function is_condition_matched_with_method_settings( array $condition_settings, ShippingContents $contents, LoggerInterface $logger, MethodSettings $method_settings ) {
		...
		$ratio = isset( $method_settings_raw[ self::RATIO ] ) ? $method_settings_raw[ self::RATIO ] : null;

		if ( ! $this->is_valid_ratio( $ratio ) ) {
	 ...
	 * @return string
	 */
	protected function format_for_log( array $condition_settings, $condition_matched, $input_data, $ratio = null ) {
	 ...
	 * @return bool
	 */
	private function is_valid_ratio( $ratio ) {

		if ( null === $ratio ) {
			return false;
		} elseif ( strlen( $ratio ) === 0 ) {
			return false;
		} elseif ( 0.0 === (float) $ratio ) {
			return false;
		}
